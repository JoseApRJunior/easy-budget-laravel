...
function generateTokenExpirate( $timeExpires = '+24 hours' )
{
...
function dateExpirate( $timeExpires = '+1 month ' )
{
...
function convertToDateTime( $dateTimeString )
{
    ...
    try {
        ...
        return DateTime::createFromFormat( 'Y-m-d H:i:s', $formatted, new DateTimeZone( env( 'APP_TIMEZONE' ) ) );
...
function convertDateLocale( $dateCreated, $locale ): DateTime
{
...
function convertToDateTimeString( $dateTimeString, $format = 'Y-m-d H:i:s' )
{
    if ( $dateTimeString === null || $dateTimeString === '' ) {
        return null;
    }
...
function mapMerchantOrderStatusMercadoPago( $merchantOrderStatus ): MerchantOrderStatusMercadoPagoEnum
{
...
function mapMerchantOrderOrderStatusMercadoPago( $merchantOrderOrderStatus ): MerchantOrderOrderStatusMercadoPagoEnum
{
 ...
 * @param array|object $originalData Array or object com os dados originais
 * @param array $excludesPropertiesData Array com as propriedades a serem excluídas
 * @param array $requestData Array com os dados da requisição
 * @return array Array  $requestData com os dados filtrados do array $originalData e removido do array $excludesPropertiesData
 */
function removeUnnecessaryIndexes( $originalData, array $excludesPropertiesData, array $requestData ): array
{
 ...
 * @return array Array com os nomes das propriedades do construtor
 */
function getConstructorProperties( $class ): array
{
 ...
 * @param array $ignoreFields Campos a serem ignorados na comparação
 * @return bool Retorna true se os objetos forem iguais, false caso contrário
 */
function compareObjects( $objLast, $objNew, $ignoreFields = [] )
{
...
function generateReportHash( $content, $data, $user_id, $tenant_id )
{
...
function generateDescriptionPipe( $data )
{
    return implode(
        ' | ',
        array_map(
...
function formatDescription( $data ): string
{
    return implode(
        ' | ',
        array_map(
...
function getTranslatedMessage( $entity = 'default', $method = 'index', $indiceParameter = 'default', $indiceValidator = 'default', $language = null ): string
{
 ...
 *
 * @param string $language O idioma desejado
