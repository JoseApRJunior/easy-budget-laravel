...
class BudgetService
{
    ...
    public function handleStatusChange(array $data, object $authenticated)
    {
        $result = [];
    ...
    private function sendNotificationBudget(object $budget, string $token)
    {
    ...
    {
        try {
            return $this->connection->transactional(function () use ($budget, $newBudgetStatuses, $services, $authenticated) {
                $result = [];
                $updatedService = [];
     ...
     *
     * @param array $service Dados do serviço a ser atualizado.
     * @param string $newStatusSlug Slug do novo status.
     * @param array $serviceStatusesBySlug Lista de status disponíveis indexados pelo slug.
    ...
    public function updateService(array $data)
    {
        try {
            return $this->connection->transactional(function () use ($data) {
                ...
                if ($serviceItem instanceof EntityNotFound) {
                    return [
                        'status' => 'error',
    ...
    public function printPDF(object $authenticated, object $customer, object $budget, array $services, array $service_items, array $latest_schedules)
    {
