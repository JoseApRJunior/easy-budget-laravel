...
class PaymentMercadoPagoPlanService
{
     ...
     *
     * @param array $webhookPaymentData Dados do pagamento recebidos do webhook.
     ...
     */
    public function getPaymentFromMercadoPagoAPI( string $paymentId ): ?Payment
    {
        try {
            $this->authenticate();
     ...
     */
    public function refundPaymentOnMercadoPago( string $paymentId ): bool
    {
        try {
            $this->authenticate();
     ...
     * @return array O resultado da operação de atualização.
     */
    ...
    public function createMercadoPagoPreference( $lastPlan, $newPlan, $plan_subscription_id )
    {
        ...
        try {
            ...
            return [ 
                'status'      => $preference ? 'success' : 'error',
                'message'     => $preference ? 'Preferençia de pagamento criada com sucesso' : 'Erro ao criar preferência de pagamento.',
                'data'        => $preference,
