...
class NotifyChangeOfStatusService
{
    /**
     * Summary of table
     * @var string
    ...
    public function sendEmailNotificationStatusService( array $service, string $token, string $template_type = 'default', $tenant_id = null )
    {
        try {
            return $this->connection->transactional( function () use ($service, $token, $template_type, $tenant_id) {
                ...
                $templates = [ 
                    ...
                    'cancelled_in_progress' => 'notification-status-cancelled-in-progress',
                ...
                $budget = $this->budget->getBudgetFullById( $service[ 'budget_id' ], $tenant_id );
                ...
                // Sessão de configuração de dados do usuário
                $customer = $this->customer->getCustomerFullbyId( $budget->customer_id, $tenant_id );
