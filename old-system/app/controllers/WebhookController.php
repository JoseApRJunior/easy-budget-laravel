    ...
    public function handleMercadoPagoInvoice(): Response
    {
        ...
        try {
            ...
            if ( !$invoiceAlreadyExists && !$invoicePaymentAlreadyExists ) {
                $authenticated = $this->providerModel->getProviderFullByUserId( $payment[ 'user_id' ], $payment[ 'tenant_id' ] );

                if ( $authenticated instanceof EntityNotFound ) {
                ...
                $customer = $this->customerModel->getCustomerFullById( $payment[ 'customer_id' ], $payment[ 'tenant_id' ] );
                if ( !$customer ) {
