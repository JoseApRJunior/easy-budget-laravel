...
class PlanController extends AbstractController
{
    ...
    public function cancelPendingSubscription(): Redirect
    {
        try {
            ...
            // todo testar se esta funcioando cancelado o pagamento pendente no mercado pago
            $mercadoPagoPaymentStatuses = [ 'pending', 'authorized', 'in_process', 'in_mediation' ];

            if ( !$mercadoPagoPayment instanceof EntityNotFound && in_array( $mercadoPagoPayment->status, $mercadoPagoPaymentStatuses ) ) {
                $wasCancelledOnMP = $this->paymentMercadoPagoPlanService->cancelPaymentOnMercadoPago( $mercadoPagoPayment->payment_id );
                if ( $wasCancelledOnMP ) {
    ...
    public function status(): Response
    {
        try {
            ...
            if ( $localPayment instanceof EntityNotFound ) {
            ...
            $mercadoPagoPayment = $this->paymentMercadoPagoPlanService->getPaymentFromMercadoPagoAPI( $localPayment->payment_id );
