...
class Provider extends Model
{
    ...
    /**
     * Cria uma nova entidade ProviderFullEntityJoin a partir de um array de dados.
     *
     * @param array<string, mixed> $data Os dados para criar a entidade
     * @return Entity A entidade ProviderFullEntityJoin criada
     */
    protected static function createEntityJoin( array $data ): Entity
    {
        return ProviderFullEntityJoin::create( $data );
    }
     ...
     * @return array<int, Entity>|Entity|ProviderEntity O prestador de serviço encontrado
     */
    ...
    protected function buildProviderFullBaseQuery(): \Doctrine\DBAL\Query\QueryBuilder
    {
    ...
    public function getProviderFullByUserId( int $user_id, int $tenant_id ): ProviderFullEntityJoin|Entity
    {
        try {
            $query = $this->buildProviderFullBaseQuery()
            ...
            return $this->createEntityJoin( $result );
        } catch ( Exception $e ) {
            throw new RuntimeException( "Falha ao buscar prestador, tente mais tarde ou entre em contato com suporte.", 0, $e );
        }
    }

    public function getProviderFullByEmail( string $email ): ProviderFullEntityJoin|Entity
    {
        try {
            $query = $this->buildProviderFullBaseQuery()
                ->where( 'u.email = :email' )
     ...
     * @param int $user_id O ID do usuário
     * @param int $tenant_id O ID do tenant
     * @return array<int, string> Um array com os nomes dos papéis do prestador
     */
     ...
     * @param int $user_id O ID do usuário
     * @param int $tenant_id O ID do tenant
     * @return array<int, string> Um array com os nomes das permissões do prestador
     */
