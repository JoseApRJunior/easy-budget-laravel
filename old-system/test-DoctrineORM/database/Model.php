...
abstract class Model implements ModelInterface
{
    ...
    // /**
    //  * Create an entity from an array of data for joins
    //  * This method should be implemented in each child class
    //  *
    //  * @param array <string,mixed> $data
    //  * @return Entity
    ...
    // abstract protected static function createEntityJoin( array $data ): Entity;
     ...
     *
     * @param array<int, array<string, mixed>> $result O array de resultado bruto da consulta ao banco de dados.
     *
     * @return array<int, Entity>|Entity|EntityNotFound Retorna um array de entidades ou uma única entidade.
     */
    ...
    /**
     * Lida com o resultado de uma consulta usando uma classe específica.
     *
     * @param array<int, array<string, mixed>> $result O array de resultado bruto da consulta
     * @param string $class A classe a ser utilizada para criar as entidades
     *
     * @return array<int, Entity>|Entity|EntityNotFound
     */
    public function handleQueryResultClass( array $result, string $class ): array|Entity
    {
        if ( count( $result ) === 1 ) {
    ...
    /**
     * Lida com o resultado de uma consulta retornando objetos.
     *
     * @param array<int, array<string, mixed>> $result O array de resultado bruto da consulta
     *
     * @return object
     ...
     * @return array{status: string, message: string, data: array<string, mixed>} Um array associativo com o resultado da operação.
     * @throws PDOException Lançada se ocorrer um erro na comunicação com o banco de dados.
     */
     ...
     *
     * @return array<int, Entity>|Entity|EntityNotFound Retorna um array de entidades, uma única entidade se houver apenas um resultado, ou EntityNotFound se a tabela estiver vazia.
     * @throws PDOException Lançada se ocorrer um erro na comunicação com o banco de dados.
     */
     ...
     * @param array<int, string> $fields Um array de colunas a serem selecionadas. O padrão é `['*']`.
     * @return array<int, Entity>|Entity|EntityNotFound Retorna uma única entidade, um array de entidades ou EntityNotFound se nada for encontrado.
     * @throws PDOException Lançada se ocorrer um erro na comunicação com o banco de dados.
     */
    ...
    /**
     * Busca uma única entidade com base em critérios de filtragem.
     *
     * Este método é um wrapper em torno de `findBy`, mas garante que apenas
     * uma entidade (ou `null`) seja retornada. É ideal para situações onde
     * se espera um único resultado, como ao buscar por um ID único.
     *
     * @param array<string, mixed> $criteria Um array associativo de `['coluna' => 'valor']` para a cláusula WHERE.
     * @param array<string, string>|null $orderBy Um array associativo para a ordenação, ex: `['nome' => 'ASC']`.
     * @return Entity|EntityNotFound Retorna a entidade encontrada ou `EntityNotFound` se nenhum registro corresponder aos critérios.
     * @throws PDOException Lançada se ocorrer um erro na comunicação com o banco de dados.
     */
    public function findOneBy( array $criteria, ?array $orderBy = null ): Entity|EntityNotFound
    {
        $result = $this->findBy( $criteria, $orderBy, 1 );

        if ( $result instanceof EntityNotFound || empty( $result ) ) {
            return new EntityNotFound();
        }
     ...
     *
     * @param array<string, mixed> $criteria Condições de busca no formato `['alias.coluna' => 'valor']`.
     * @param array<int, array<string, string>> $joins Configurações de JOIN. Cada join é um array com 'type', 'table', 'alias', e 'condition'.
     *                     Exemplo: `[['type' => 'innerJoin', 'table' => 'users', 'alias' => 'u', 'condition' => 'main.user_id = u.id']]`
     * @param array<string, string>|null $orderBy Ordenação dos resultados no formato `['alias.coluna' => 'ASC|DESC']`.
     * @param int|null $limit Limite de resultados a serem retornados.
     * @param int|null $offset Offset para paginação.
     * @param array<int, string> $fields Campos a serem selecionados. Ex: `['main.id', 'u.name']`.
     * @return array<int, Entity>|Entity|EntityNotFound Retorna um array de entidades, uma única entidade, ou EntityNotFound.
     * @throws PDOException Lançada se ocorrer um erro na comunicação com o banco de dados.
     */
     ...
     * @return array<string, string> Um array associativo `['nome_da_propriedade' => 'tipo_doctrine']`.
     */
     ...
     *
     * @return array<string, mixed> Um array de objetos Column.
     */
