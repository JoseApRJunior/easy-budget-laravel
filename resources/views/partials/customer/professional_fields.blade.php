...
<div class="mb-3">
    ...
    <input type="email_business" class="form-control @error( 'email_business' ) is-invalid @enderror" id="email_business"
        name="email_business" value="{{ old( 'email_business', $customer->email_business ?? '' ) }}" required>
...
<div class="mb-3">
    ...
    <select name="area_of_activity_id" class="form-control @error( 'area_of_activity_id' ) is-invalid @enderror"
        ...
        @foreach ( $areas_of_activity as $area )
            <option value="{{ $area->id }}" {{ old( 'area_of_activity_id', $customer->area_of_activity_id ?? '' ) == $area->id ? 'selected' : '' }}>
                {{ $area->name }}
            </option>
        @endforeach
    </select>
...
<div class="mb-3">
    ...
    <select name="profession_id" class="form-control @error( 'profession_id' ) is-invalid @enderror" id="profession"
        ...
        @foreach ( $professions as $prof )
            <option value="{{ $prof->id }}" {{ old( 'profession_id', $customer->profession_id ?? '' ) == $prof->id ? 'selected' : '' }}>
                {{ $prof->name }}
            </option>
        @endforeach
    </select>
