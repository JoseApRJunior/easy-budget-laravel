    ...
    <div class="row row-cols-1 row-cols-md-3 g-4 mb-5">
      ...
      <div class="col">
        <div class="card h-100 shadow-sm hover-card">
          ...
          <div class="card-body d-flex flex-column">
            <div class="text-center mb-4">
              ...
              <div class="pricing-header">
                ...
                <span class="price">{{ number_format( entityProperty( $plan, 'price' ), 0, '', '.' ) }}</span>
            ...
            <ul class="feature-list list-unstyled mb-4">
              @foreach( entityProperty( $plan, 'features' ) as $feature )
              <li class="mb-2">
          ...
          echo checkFeature( 'plan-listing', $cardContent, entityProperty( $plan, 'slug' ) != 'free' );
          @endphp
        </div>
      </div>
      @endforeach
          ...
          <div class="row g-3">
            <div class="col-md-3">
              ...
              <select class="form-select" id="planSelect" name="plan" required>
                @foreach( $plans as $plan )
                <option value="{{ entityProperty( $plan, 'slug' ) }}"
                  {{ entityProperty( $plan, 'slug' ) != 'free' ? 'disabled' : '' }}>
                  {{ entityProperty( $plan, 'name' ) }} - R$
                  {{ number_format( entityProperty( $plan, 'price' ), 2, ',', '.' ) }}
                  {{ entityProperty( $plan, 'slug' ) != 'free' ? ' - em desenvolvimento' : '' }}
                </option>
                @endforeach
              </select>
              @error( 'plan' )
