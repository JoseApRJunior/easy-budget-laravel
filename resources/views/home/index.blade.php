...
@section( 'title', 'Easy Budget - Sistema de Orçamentos' )
@section( 'description', 'Transforme a gestão de seus serviços com nossas soluções diversificadas e inovadoras.' )
                        ...
                        <div class="card h-100 shadow-sm hover-card">
                            <div class="card-body d-flex flex-column">
                                <div class="text-center mb-4">
                                    @if( strtolower( $plan['slug'] ?? '' ) == 'free' )
                                        <i class="bi bi-rocket display-6 text-primary mb-2"></i>
                                    @elseif( strtolower( $plan['slug'] ?? '' ) == 'basic' )
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.select-plan').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('planSelect').value = planName.toLowerCase().replace(' ', '_');

                // Scroll para o formulário
                document.getElementById('preCadastroForm').scrollIntoView({
                    behavior: 'smooth'
                });
            });
        });
        ...
        passwordInput.addEventListener('input', function() {
            ...
            // Verificar regras da senha
            const rules = passwordRules.querySelectorAll('li');
            rules[0].querySelector('i').style.color = password.length >= 6 ? '#28a745' : '#ccc'; // 6 caracteres
            rules[1].querySelector('i').style.color = /[a-z]/.test(password) ? '#28a745' : '#ccc'; // minúsculas
            rules[2].querySelector('i').style.color = /[A-Z]/.test(password) ? '#28a745' : '#ccc'; // maiúsculas
            rules[3].querySelector('i').style.color = /\d/.test(password) ? '#28a745' : '#ccc'; // números
            rules[4].querySelector('i').style.color = /[@#$!%*?&]/.test(password) ? '#28a745' : '#ccc'; // especiais
        });
    });
    </script>
@endsection
