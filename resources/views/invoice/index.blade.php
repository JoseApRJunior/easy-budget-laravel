...
<div class="container-fluid py-4">
    ...
    <div class="col-md-6">
      <div class="card h-100 shadow-sm border-0 hover-shadow">
        <div class="card-body d-flex flex-column">
          ...
          <a href="/provider/reports/invoices" class="btn btn-primary">
            Acessar Relatório
          </a>
        </div>
      </div>
    ...
    <div class="col-md-6">
      <div class="card h-100 shadow-sm border-0 hover-shadow">
        <div class="card-body d-flex flex-column">
          ...
          <a href="/provider/services" class="btn btn-info text-white">
            Ver Serviços
          </a>
        </div>
      </div>
  ...
  <div class="card border-0 shadow-sm mb-4">
    <div class="card-body p-4">
      <form id="filter-form" class="row g-3">
        ...
        <div class="col-md-2">
          <div class="form-floating">
            <input type="text" class="form-control " id="code" name="code" placeholder="Nº Fatura"
              value="{{ $filters[ 'code' ] ?? '' }}">
            <label for="code">
              <i class="bi bi-hash me-1"></i>Nº Fatura
            </label>
          </div>
        </div>
        ...
        <div class="col-md-2">
          <div class="form-floating">
            <input type="text" class="form-control" id="customer_name" name="customer_name" placeholder="Cliente"
              value="{{ $filters[ 'customer_name' ] ?? '' }}" autocomplete="off">
        ...
        <div class="col-md-2">
          <div class="form-floating">
            <input type="text" class="form-control money-input" id="total" name="total" placeholder="Valor Mínimo (R$)"
              value="{{ $filters[ 'total' ] ? number_format( $filters[ 'total' ], 2, ',', '.' ) : '' }}"
              autocomplete="off" maxlength="20">
            <label for="total">
              <i class="bi bi-currency-dollar me-1"></i>Valor Mínimo (R$)
            </label>
          </div>
        </div>
          ...
          <div class="form-floating">
            <select class="form-select" id="status" name="status">
              ...
              <option value="{{ $status }}" {{ $status == $filters[ 'status' ] ? 'selected' : '' }}>
                {{ $invoice_statuses[ $status ] ?? $status }}
              </option>
              @endforeach
              @else
              <option value="{{ $filters[ 'status' ] }}" selected>
                {{ $invoice_statuses[ $filters[ 'status' ] ] ?? $filters[ 'status' ] }}
              </option>
              @endif
              @endif
              @foreach( $invoice_statuses as $key => $label )
  ...
  <!-- Mensagem Inicial -->
  <div id="initial-message" class="card border-0 shadow-sm text-center py-5 {{ isset( $invoices ) ? 'd-none' : '' }}">
    <div class="card-body">
    ...
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-transparent border-0">
        <div class="d-flex justify-content-between align-items-center">
          <h2 class="h5 mb-0">
            <i class="bi bi-list me-2"></i>Faturas Encontradas
          </h2>
