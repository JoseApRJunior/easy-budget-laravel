                    ...
                    @if(isset($log_entries) && count($log_entries) > 0)
                        <div class="log-container" style="max-height: 600px; overflow-y: auto;">
                            @foreach($log_entries as $log)
                                <div class="log-entry" data-level="{{ $log['level'] ?? 'info' }}" data-provider="{{ $log['provider'] ?? '' }}">
                                        ...
                                        <div class="log-content flex-grow-1">
                                            <div class="d-flex justify-content-between align-items-start mb-1">
                                                <div class="log-header">
                                                    ...
                                                    <span class="badge bg-{{ ($log['level'] ?? 'info') === 'error' ? 'danger' : (($log['level'] ?? 'info') === 'warning' ? 'warning' : 'info') }} ms-2">
                                                        {{ strtoupper($log['level'] ?? 'INFO') }}
                                                    </span>
                                                    @if(isset($log['provider']))
                                                        <span class="badge bg-secondary ms-1">{{ $log['provider'] }}</span>
                                                    @endif
                                                </div>
                                                    ...
                                                    <div class="mt-1">
                                                        @foreach($log['context'] as $key => $value)
                                                            <span class="badge bg-light text-dark me-1 mb-1">
                                                                {{ $key }}: {{ is_array($value) || is_object($value) ? json_encode($value) : $value }}
                                                            </span>
                                                        @endforeach
                                                    </div>
                                                </div>
