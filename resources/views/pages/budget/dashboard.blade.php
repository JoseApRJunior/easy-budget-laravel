...
@section( 'content' )
  <div class="container-fluid py-4">
    ...
    @php
      ...
      $approvedRate = $total > 0
        ? number_format( ( $approved / $total ) * 100, 1, ',', '.' )
        : 0;
    @endphp
                    ...
                    @foreach( $recent as $budget )
                      @php
                        ...
                        $statusBadge = match ( $budget->status ?? 'pending' ) {
                          \App\Enums\BudgetStatus::APPROVED->value   => '<span class="badge bg-success-subtle text-success">Aprovado</span>',
                          \App\Enums\BudgetStatus::REJECTED->value   => '<span class="badge bg-danger-subtle text-danger">Rejeitado</span>',
                          \App\Enums\BudgetStatus::COMPLETED->value   => '<span class="badge bg-info-subtle text-info">Concluído</span>',
                          \App\Enums\BudgetStatus::CANCELLED->value   => '<span class="badge bg-secondary-subtle text-secondary">Cancelado</span>',
                          \App\Enums\BudgetStatus::DRAFT->value   => '<span class="badge bg-light-subtle text-muted">Rascunho</span>',
                          default => '<span class="badge bg-warning-subtle text-warning">Pendente</span>'
                        };
                      @endphp
                      <tr>
                        <td>{{ $budget->code }}</td>
