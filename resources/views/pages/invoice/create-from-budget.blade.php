        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6 text-md-end">
                    ...
                    <div><strong>Saldo Disponível:</strong> <span id="remaining-balance" class="text-{{ $remainingBalance > 0 ? 'success' : 'danger' }}">R$ {{ number_format($remainingBalance, 2, ',', '.') }}</span></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="alert alert-info">
                    <strong>Selecione os itens para faturar:</strong> O total selecionado não pode exceder o saldo disponível de <strong>R$ {{ number_format($remainingBalance, 2, ',', '.') }}</strong>
                </div>
                            ...
                            @foreach($budget->services as $service)
                                @foreach($service->serviceItems as $item)
                                    <tr data-item-id="{{ $item->id }}" data-item-total="{{ $item->total }}">
                                        <td>
                                            <input type="checkbox" class="form-check-input item-checkbox" name="items[{{ $item->id }}][selected]" value="1" data-item-id="{{ $item->id }}" />
                                            <input type="hidden" name="items[{{ $item->id }}][service_item_id]" value="{{ $item->id }}" />
                                        </td>
                                        <td>{{ $service->description }}</td>
                                        <td>{{ $item->product->name ?? ('#'.$item->product_id) }}</td>
                                        <td>
                                            <input type="number" step="0.01" min="0.01" max="{{ $item->quantity }}" class="form-control form-control-sm text-end item-quantity" name="items[{{ $item->id }}][quantity]" value="{{ $item->quantity }}" data-item-id="{{ $item->id }}" data-original-quantity="{{ $item->quantity }}" />
                                        </td>
                                        <td>
                                            <input type="number" step="0.01" min="0.01" class="form-control form-control-sm text-end item-unit-value" name="items[{{ $item->id }}][unit_value]" value="{{ $item->unit_value }}" data-item-id="{{ $item->id }}" data-original-unit-value="{{ $item->unit_value }}" />
                                        </td>
                                        <td class="text-end original-total">R$ {{ number_format($item->total, 2, ',', '.') }}</td>
                                        <td class="text-end selected-total" data-item-id="{{ $item->id }}">R$ 0,00</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateItemTotal(itemId) {
        ...
        const unitValue = parseFloat(document.querySelector(`.item-unit-value[data-item-id="${itemId}"]`).value) || 0;
    ...
    function updateSelectedTotal() {
        ...
        itemCheckboxes.forEach(checkbox => {
            ...
            const selectedTotalElement = document.querySelector(`.selected-total[data-item-id="${itemId}"]`);
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        if (!confirm(`Confirma a criação da fatura no valor de ${formatCurrency(totalSelected)}?`)) {
            e.preventDefault();
        }
    });
