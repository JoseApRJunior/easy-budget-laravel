...
class BudgetBulkUpdateStatusFormRequestTest extends TestCase
{
    ...
    /** @test */
    public function it_validates_status_exists_in_budget_statuses_table()
    {
        $request = new BudgetBulkUpdateStatusFormRequest();
        $rules = $request->rules();
    ...
    public function it_validates_notify_customers_is_optional_boolean()
    {
        ...
        $errors = $validator->errors()->toArray();
        $this->assertArrayNotHasKey('notifyCustomers', $errors);
        ...
        $validator = Validator::make([
            'budget_ids' => [1],
            'status' => 'approved',
            'notifyCustomers' => 'not-boolean',
        ], $rules);

        $this->assertFalse($validator->passes());
        $this->assertArrayHasKey('notifyCustomers', $validator->errors()->toArray());
    }
    ...
    public function it_provides_custom_error_messages()
    {
        ...
        $this->assertArrayHasKey('status.exists', $messages);
        $this->assertArrayHasKey('comment.string', $messages);
        $this->assertArrayHasKey('comment.max', $messages);
        $this->assertArrayHasKey('notifyCustomers.boolean', $messages);
    ...
    public function it_provides_custom_attribute_names()
    {
        ...
        $this->assertArrayHasKey('notifyCustomers', $attributes);
        ...
        $this->assertEquals('Notificar clientes', $attributes['notifyCustomers']);
    }
    ...
    public function it_validates_with_all_valid_budget_statuses()
    {
        ...
        $validStatuses = BudgetStatus::pluck('slug')->toArray();
