...
try {
    ...
    // Testar ServiceService com tenant scoping correto - com debug detalhado
    ...
    try {
        ...
        if ($result->isSuccess()) {
            ...
            if ($result->getException()) {
                echo "- Exceção: " . $result->getException()->getMessage() . "\n";
                echo "- Stack trace: " . $result->getException()->getTraceAsString() . "\n";
            }
        }
    } catch (\Exception $e) {
    ...
    // Testar carregamento manual de relacionamentos
    echo "\nTestando carregamento manual de relacionamentos:\n";
    try {
        $service->load(['category', 'serviceItems.product']);
        echo "✓ Relacionamentos básicos carregados com sucesso\n";
        ...
        if ($service->serviceItems->count() > 0) {
            foreach ($service->serviceItems as $item) {
                echo "  - Item ID: {$item->id}, Produto: " . ($item->product ? $item->product->name : "Sem produto") . "\n";
            }
        }
