...
/**
 * Repositório para gerenciamento de status de faturas usando enums.
 *
 * Esta classe implementa GlobalRepositoryInterface para fornecer operações
 * compatíveis com InvoiceStatusEnum, que substitui o modelo InvoiceStatus.
 * Como os status agora são enums, algumas operações não são aplicáveis.
 *
 * @package App\Repositories
 */
class InvoiceStatusRepository implements GlobalRepositoryInterface
{
    /**
     * Busca status de fatura por slug.
     *
     * @param string $slug Slug único do status
     * @return InvoiceStatusEnum|null Status encontrado ou null se não existir
     */
    public function findBySlug( string $slug ): ?InvoiceStatusEnum
    {
        return InvoiceStatusEnum::tryFrom( $slug );
    }
     ...
     * @param int|null $limit Limite de registros (opcional)
     * @return array<InvoiceStatusEnum> Lista ordenada de status
     */
    public function findOrderedBy( string $field, string $direction = 'asc', ?int $limit = null ): array
    {
        $allStatuses = InvoiceStatusEnum::cases();
     ...
     * @return InvoiceStatusEnum|null Status encontrado ou null se não existir
     */
    public function findByName( string $name ): ?InvoiceStatusEnum
    {
    ...
    {
        return InvoiceStatusEnum::tryFrom( $slug ) !== null;
    }
     ...
     * @return array<InvoiceStatusEnum> Lista de status com a cor especificada
     */
    public function findByColor( string $color ): array
    {
        return array_filter(
            InvoiceStatusEnum::cases(),
            fn( InvoiceStatusEnum $status ) => $status->getColor() === $color
        );
    }
     ...
     * @return array<InvoiceStatusEnum> Lista de status no range especificado
     */
    public function findByOrderIndexRange( int $minOrderIndex, int $maxOrderIndex ): array
    {
        return array_filter(
            InvoiceStatusEnum::cases(),
            fn( InvoiceStatusEnum $status ) =>
     ...
     */
    public function findById( int $id ): ?InvoiceStatusEnum
    {
        ...
        return $slug ? InvoiceStatusEnum::tryFrom( $slug ) : null;
    }
     ...
     */
    public function findOneBy( array $criteria ): ?InvoiceStatusEnum
    {
    ...
    {
        throw new LogicException( 'Cannot create new InvoiceStatus - it is now an enum' );
    }
    ...
    {
        throw new LogicException( 'Cannot update InvoiceStatus - it is now an enum' );
    }
    ...
    {
        throw new LogicException( 'Cannot delete InvoiceStatus - it is now an enum' );
    }
     ...
     */
    private function enumToModel( InvoiceStatusEnum $status ): Model
    {
        return new class ($status) extends Model
        {
            public InvoiceStatusEnum $enum;

            public function __construct( InvoiceStatusEnum $enum )
            {
                $this->enum = $enum;
            }
