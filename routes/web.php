...
use App\Http\Controllers\DebugTenantController;
...
Route::group([], function () {
    // Rota temporária para diagnóstico de e-mail em produção
    Route::get('/debug-email-prod', function () {
        if (request('key') !== 'easy-debug-123') {
            abort(403, 'Acesso negado.');
        }
        
        echo "<pre>";
        require base_path('debug_mail.php');
        echo "</pre>";
    });
...
Route::prefix('p')->name('provider.')->middleware(['auth', 'verified', 'provider', 'monitoring'])->group(function () {
    // Debug Tenant Access
    Route::get('/debug-tenant', [DebugTenantController::class, 'index'])->name('debug-tenant');
...
Route::prefix('a')->name('admin.')->middleware(['auth', 'admin', 'monitoring'])->group(function () {
    ...
    Route::prefix('alerts')->name('alerts.')->group(function () {
        ...
        Route::get('/{alert}', [AlertsController::class, 'show'])->name('show');
        Route::get('/{alert}/edit', [AlertsController::class, 'edit'])->name('edit');
        Route::put('/{alert}', [AlertsController::class, 'update'])->name('update');
        Route::delete('/{alert}', [AlertsController::class, 'destroy'])->name('destroy');
        Route::post('/{alert}/toggle-status', [AlertsController::class, 'toggleStatus'])->name('toggle-status');
        Route::get('/export/{format}', [AlertsController::class, 'export'])->name('export');
    });
    ...
    Route::prefix('audit')->name('audit.')->group(function () {
        ...
        Route::get('/logs/{log}', [AuditController::class, 'show'])->name('logs.show');
        Route::get('/export', [AuditController::class, 'export'])->name('logs.export');
        Route::delete('/logs/{log}', [AuditController::class, 'destroy'])->name('logs.destroy');
    });
});
...
Route::prefix('reports')->name('reports.')->middleware(['auth', 'verified'])->group(function () {
    ...
    // PDF/Excel export routes for specific reports
    Route::get('/budgets/pdf', [ReportController::class, 'budgets_pdf'])->name('budgets.pdf');
    Route::get('/budgets/excel', [ReportController::class, 'budgets_excel'])->name('budgets.excel');
    Route::get('/customers/pdf', [ReportController::class, 'customersPdf'])->name('customers.pdf');

    // API routes for reports
    Route::get('/customers/search', [ReportController::class, 'customersSearch'])->name('customers.search');
});
...
Route::middleware(['auth', 'verified', 'provider'])->group(function () {
    Route::redirect('/p/update', '/p/business/edit')->name('provider.update.redirect');
