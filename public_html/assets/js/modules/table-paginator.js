...
class TablePaginator {
   ...
   updateTable(items) {
      if (!this.tableBody) {
         console.error(
            `Elemento tbody da tabela #${this.tableId} não encontrado!`
         );
         return;
      }
   ...
   addPaginationEventListeners(totalPages) {
      const self = this;
      document
         .querySelectorAll(`#${this.paginationId} .page-link`)
         .forEach((link) => {
   ...
   formatItems(items) {
      if (!items || !items.length) {
         return `
            <tr>
                <td colspan="${this.colSpan}" class="text-center py-1">
